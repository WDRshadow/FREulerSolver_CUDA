...
__host__ __device__ inline double pressure(const Vec4 &Q, double gamma)
{
    return (gamma - 1.0) * (_e - 0.5 * _rho * (_u * _u + _v * _v));
}

__host__ __device__ inline Vec4 toPrimitive(const Vec4 &Q, double gamma)
{
...
__host__ __device__ inline Flux physicalFlux(const Vec4 &Q, double gamma)
{
    Vec4 Fx, Fy;
    Fx[0] = _rho * _u;
    Fx[1] = _rho * _u * _u + _p;
